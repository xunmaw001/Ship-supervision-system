            ...
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '材料名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.yanshouCailiaoName" placeholder="材料名称" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('yanshouCailiao','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/chuanbo/upload/yanshouCailiaoMuBan.xls"
                        >批量导入材料验收数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('yanshouCailiao','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="yanshouCailiaoUploadSuccess"
                                :on-error="yanshouCailiaoUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('yanshouCailiao','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入材料验收数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('yanshouCailiao','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "yanshouCailiao.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
...
<!--                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="yanshouCailiaoName"
                                   header-align="center"
                                   label="材料名称">
                        <template slot-scope="scope">
                            {{scope.row.yanshouCailiaoName}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="yanshouCailiaoBuwei"
                                   header-align="center"
                                   label="监造部位">
                        <template slot-scope="scope">
                            {{scope.row.yanshouCailiaoBuwei}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="yanshouCailiaoBiaozhun"
                                   header-align="center"
                                   label="使用标准">
                        <template slot-scope="scope">
                            {{scope.row.yanshouCailiaoBiaozhun}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="yanshouCailiaoShijishiyong"
                                   header-align="center"
                                   label="实际使用">
                        <template slot-scope="scope">
                            {{scope.row.yanshouCailiaoShijishiyong}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('yanshouCailiao','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('yanshouCailiao','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('yanshouCailiao','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                ...
                xiangmuId : null,
                yuangongId : null,
                yanshouCailiaoName : null,
                yanshouCailiaoBuwei : null,
                yanshouCailiaoBiaozhun : null,
                yanshouCailiaoShijishiyong : null,
                jianzaojinduFile : null,
                hegeTypes : null,
                yanshouTime : null,
                yanshouCailiaoContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '账户状态': 'jinyongTypes',
                //本表字段
                     '材料名称': "yanshouCailiaoName",
                     '监造部位': "yanshouCailiaoBuwei",
                     '使用标准': "yanshouCailiaoBiaozhun",
                     '实际使用': "yanshouCailiaoShijishiyong",
                     '附件': "jianzaojinduFile",
                     '是否合格': "hegeValue",
                     '验收时间': "yanshouTime",
                     '录入时间': "insertTime",
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'yanshou_cailiao',//当前表表名,
                        sumColum : 'yanshou_cailiao_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'yanshou_cailiao_name',//分组字符串字段
                        // types : 'yanshou_cailiao_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "yanshou_cailiao_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.yanshouCailiaoName!= '' && this.searchForm.yanshouCailiaoName!= undefined) {
                    params['yanshouCailiaoName'] = '%' + this.searchForm.yanshouCailiaoName + '%'
                }
                ...
                params['yanshouCailiaoDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "yanshouCailiao/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            yanshouCailiaoUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "yanshouCailiao/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            yanshouCailiaoUploadError(data){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "yanshouCailiao/update",
                        method: "post",
                        data: {
                            id:id,
//                            yanshouCailiaoTypes:yanshouCailiaoTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
