            ...
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '进度标题' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.jianzaojinduName" placeholder="进度标题" clearable></el-input>
                     </el-form-item>
         
                     <el-form-item :label="contents.inputTitle == 1 ? '进度类型' : ''">
                        <el-select v-model="searchForm.jianzaojinduTypes" placeholder="请选择进度类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in jianzaojinduTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('jianzaojindu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/chuanbo/upload/jianzaojinduMuBan.xls"
                        >批量导入监造进度数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('jianzaojindu','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="jianzaojinduUploadSuccess"
                                :on-error="jianzaojinduUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('jianzaojindu','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入监造进度数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('jianzaojindu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "jianzaojindu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
...
<!--                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jianzaojinduUuidNumber"
                                   header-align="center"
                                   label="监造进度编号">
                        <template slot-scope="scope">
                            {{scope.row.jianzaojinduUuidNumber}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jianzaojinduName"
                                   header-align="center"
                                   label="进度标题">
                        <template slot-scope="scope">
                            {{scope.row.jianzaojinduName}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="jianzaojinduTypes"
                                      header-align="center"
                                      label="进度类型">
                        <template slot-scope="scope">
                            {{scope.row.jianzaojinduValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="jianzaojinduZhengchangNumber"
                                      header-align="center"
                                      label="正常进度(%)">
                        <template slot-scope="scope">
                            {{scope.row.jianzaojinduZhengchangNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="jianzaojinduXianzaiNumber"
                                      header-align="center"
                                      label="现在进度(%)">
                        <template slot-scope="scope">
                            {{scope.row.jianzaojinduXianzaiNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jianzaojinduTime"
                                   header-align="center"
                                   label="记录时间">
                        <template slot-scope="scope">
                            {{scope.row.jianzaojinduTime}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('jianzaojindu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('jianzaojindu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('jianzaojindu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              jianzaojinduTypesSelectSearch : [],
            form:{
                id : null,
                xiangmuId : null,
                yuangongId : null,
                jianzaojinduUuidNumber : null,
                jianzaojinduName : null,
                jianzaojinduTypes : null,
                jianzaojinduZhengchangNumber : null,
                jianzaojinduXianzaiNumber : null,
                jianzaojinduFile : null,
                jianzaojinduTime : null,
                jianzaojinduContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '账户状态': 'jinyongTypes',
                //本表字段
                     '监造进度编号': "jianzaojinduUuidNumber",
                     '进度标题': "jianzaojinduName",
                     '进度类型': "jianzaojinduValue",
                     '正常进度(%)': "jianzaojinduZhengchangNumber",
                     '现在进度(%)': "jianzaojinduXianzaiNumber",
                     '附件': "jianzaojinduFile",
                     '记录时间': "jianzaojinduTime",
                     '上传时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'jianzaojindu',//当前表表名,
                        sumColum : 'jianzaojindu_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'jianzaojindu_name',//分组字符串字段
                        // types : 'jianzaojindu_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "jianzaojindu_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.jianzaojinduName!= '' && this.searchForm.jianzaojinduName!= undefined) {
                    params['jianzaojinduName'] = '%' + this.searchForm.jianzaojinduName + '%'
                }
         
                if (this.searchForm.jianzaojinduTypes!= '' && this.searchForm.jianzaojinduTypes!= undefined) {
                    params['jianzaojinduTypes'] = this.searchForm.jianzaojinduTypes
                }
                                                        
                params['jianzaojinduDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "jianzaojindu/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=jianzaojindu_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.jianzaojinduTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            jianzaojinduUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "jianzaojindu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
