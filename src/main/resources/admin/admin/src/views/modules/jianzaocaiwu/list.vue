            ...
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '花费标题' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.jianzaocaiwuName" placeholder="花费标题" clearable></el-input>
                     </el-form-item>
         
                     <el-form-item :label="contents.inputTitle == 1 ? '消费类型' : ''">
                        <el-select v-model="searchForm.jianzaocaiwuTypes" placeholder="请选择消费类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in jianzaocaiwuTypesSelectSearch"
                               v-bind:key="index"
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.jianzaocaiwuMingcheng" placeholder="名称" clearable></el-input>
                     </el-form-item>
                                         
                     <el-form-item :label="contents.inputTitle == 1 ? '是否合格' : ''">
                        <el-select v-model="searchForm.hegeTypes" placeholder="请选择是否合格">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in hegeTypesSelectSearch"
                               v-bind:key="index"
                               :label="item.indexName"
                               :value="item.codeIndex">
                            <!--lable是要展示的名称-->
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('jianzaocaiwu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/chuanbo/upload/jianzaocaiwuMuBan.xls"
                        >批量导入监造财务数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('jianzaocaiwu','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="jianzaocaiwuUploadSuccess"
                                :on-error="jianzaocaiwuUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('jianzaocaiwu','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入监造财务数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('jianzaocaiwu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "jianzaocaiwu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
              ...
              <div style="float:right;width: 100%;text-align: right;font-size: 34px;margin-top:-80px;color: red;" >{{sumJine}}</div>
            </el-form>
...
<!--                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jianzaocaiwuUuidNumber"
                                   header-align="center"
                                   label="监造财务编号">
                        <template slot-scope="scope">
                            {{scope.row.jianzaocaiwuUuidNumber}}
                        </template>
                    </el-table-column>-->

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jianzaocaiwuName"
                                   header-align="center"
                                   label="花费标题">
                        <template slot-scope="scope">
                            {{scope.row.jianzaocaiwuName}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="jianzaocaiwuTypes"
                                      header-align="center"
                                      label="消费类型">
                        <template slot-scope="scope">
                            {{scope.row.jianzaocaiwuValue}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="jianzaojinduFile"
                                     header-align="center"
                                     width="200"
                                     label="附件">
                        <template slot-scope="scope">
                            <div v-if="scope.row.jianzaojinduFile">
                                <a type="text" style="text-decoration:none" class="el-button" :href="$base.url+scope.row.jianzaojinduFile">下载</a>
                            </div>
                            <div v-else>无文件</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jianzaocaiwuMingcheng"
                                   header-align="center"
                                   label="名称">
                        <template slot-scope="scope">
                            {{scope.row.jianzaocaiwuMingcheng}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jianzaocaiwuDanjia"
                                   header-align="center"
                                   label="单价">
                        <template slot-scope="scope">
                            {{scope.row.jianzaocaiwuDanjia}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="jianzaocaiwuNumber"
                                      header-align="center"
                                      label="数量">
                        <template slot-scope="scope">
                            {{scope.row.jianzaocaiwuNumber}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jianzaocaiwuZongjine"
                                   header-align="center"
                                   label="应金额">
                        <template slot-scope="scope">
                            {{scope.row.jianzaocaiwuZongjine}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jianzaocaiwuShijiZongjine"
                                   header-align="center"
                                   label="实际金额">
                        <template slot-scope="scope">
                            {{scope.row.jianzaocaiwuShijiZongjine}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="hegeTypes"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('jianzaocaiwu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('jianzaocaiwu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('jianzaocaiwu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              jianzaocaiwuTypesSelectSearch : [],
              hegeTypesSelectSearch : [],
            form:{
                id : null,
                xiangmuId : null,
                yuangongId : null,
                jianzaocaiwuUuidNumber : null,
                jianzaocaiwuName : null,
                jianzaocaiwuTypes : null,
                jianzaojinduFile : null,
                jianzaocaiwuMingcheng : null,
                jianzaocaiwuDanjia : null,
                jianzaocaiwuNumber : null,
                jianzaocaiwuZongjine : null,
                jianzaocaiwuShijiZongjine : null,
                hegeTypes : null,
                jianzaocaiwuContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '账户状态': 'jinyongTypes',
                //本表字段
                     '监造财务编号': "jianzaocaiwuUuidNumber",
                     '花费标题': "jianzaocaiwuName",
                     '消费类型': "jianzaocaiwuValue",
                     '附件': "jianzaojinduFile",
                     '名称': "jianzaocaiwuMingcheng",
                     '单价': "jianzaocaiwuDanjia",
                     '数量': "jianzaocaiwuNumber",
                     '应金额': "jianzaocaiwuZongjine",
                     '实际金额': "jianzaocaiwuShijiZongjine",
                     '是否合格': "hegeValue",
                     '验收时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'jianzaocaiwu',//当前表表名,
                        sumColum : 'jianzaocaiwu_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'jianzaocaiwu_name',//分组字符串字段
                        // types : 'jianzaocaiwu_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "jianzaocaiwu_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.jianzaocaiwuName!= '' && this.searchForm.jianzaocaiwuName!= undefined) {
                    params['jianzaocaiwuName'] = '%' + this.searchForm.jianzaocaiwuName + '%'
                }
         
                if (this.searchForm.jianzaocaiwuTypes!= '' && this.searchForm.jianzaocaiwuTypes!= undefined) {
                    params['jianzaocaiwuTypes'] = this.searchForm.jianzaocaiwuTypes
                }
                 
                if (this.searchForm.jianzaocaiwuMingcheng!= '' && this.searchForm.jianzaocaiwuMingcheng!= undefined) {
                    params['jianzaocaiwuMingcheng'] = '%' + this.searchForm.jianzaocaiwuMingcheng + '%'
                }
                                         
                if (this.searchForm.hegeTypes!= '' && this.searchForm.hegeTypes!= undefined) {
                    params['hegeTypes'] = this.searchForm.hegeTypes
                }
                        
                params['jianzaocaiwuDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "jianzaocaiwu/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                      ...
                      this.sumJine="目前花费:"+data.sumJine;
                    }else{
                      this.dataList = [];
                      this.totalPage = 0;
                      this.sumJine=0.0;
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=jianzaocaiwu_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.jianzaocaiwuTypesSelectSearch = data.data.list;
                    }
                });
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=hege_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.hegeTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            jianzaocaiwuUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "jianzaocaiwu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
