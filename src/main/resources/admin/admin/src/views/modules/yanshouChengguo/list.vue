        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '验收部位' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.yanshouChengguoBuwei" placeholder="验收部位" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('yanshouChengguo','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('yanshouChengguo','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('yanshouChengguo','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('yanshouChengguo','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/chuanbo/upload/yanshouChengguoMuBan.xls"
                        >批量导入成果验收数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('yanshouChengguo','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="yanshouChengguoUploadSuccess"
                                :on-error="yanshouChengguoUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('yanshouChengguo','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入成果验收数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('yanshouChengguo','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "yanshouChengguo.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('yanshouChengguo','查看')"
                          :data="dataList"
...
<!--                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="yanshouChengguoBuwei"
                                   header-align="center"
                                   label="验收部位">
                        <template slot-scope="scope">
                            {{scope.row.yanshouChengguoBuwei}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="yanshouChengguoChuandong"
                                   header-align="center"
                                   label="船东要求">
                        <template slot-scope="scope">
                            {{scope.row.yanshouChengguoChuandong}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="yanshouChengguoShiji"
                                   header-align="center"
                                   label="实际结果">
                        <template slot-scope="scope">
                            {{scope.row.yanshouChengguoShiji}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('yanshouChengguo','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('yanshouChengguo','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('yanshouChengguo','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                ...
                xiangmuId : null,
                yuangongId : null,
                yanshouChengguoBuwei : null,
                yanshouChengguoChuandong : null,
                yanshouChengguoShiji : null,
                jianzaojinduFile : null,
                hegeTypes : null,
                yanshouTime : null,
                yanshouChengguoContent : null,
                yanshouChengguoBeizhuContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '账户状态': 'jinyongTypes',
                //本表字段
                     '验收部位': "yanshouChengguoBuwei",
                     '船东要求': "yanshouChengguoChuandong",
                     '实际结果': "yanshouChengguoShiji",
                     '附件': "jianzaojinduFile",
                     '是否合格': "hegeValue",
                     '验收时间': "yanshouTime",
                     '录入时间': "insertTime",
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'yanshou_chengguo',//当前表表名,
                        sumColum : 'yanshou_chengguo_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'yanshou_chengguo_name',//分组字符串字段
                        // types : 'yanshou_chengguo_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "yanshou_chengguo_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.yanshouChengguoBuwei!= '' && this.searchForm.yanshouChengguoBuwei!= undefined) {
                    params['yanshouChengguoBuwei'] = '%' + this.searchForm.yanshouChengguoBuwei + '%'
                }
                ...
                params['yanshouChengguoDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "yanshouChengguo/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            yanshouChengguoUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "yanshouChengguo/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            yanshouChengguoUploadError(data){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "yanshouChengguo/update",
                        method: "post",
                        data: {
                            id:id,
//                            yanshouChengguoTypes:yanshouChengguoTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
