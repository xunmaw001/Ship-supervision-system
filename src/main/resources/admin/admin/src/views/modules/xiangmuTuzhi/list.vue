<template>
    <div class="main-content">
        <el-dialog title="审核" :visible.sync="xiangmuTuzhiYesnoTypesVisible">
            <el-form :model="form">
                <input type="hidden" v-model="form.id">
                <el-form-item label="审核" >
                    <el-select v-model="form.xiangmuTuzhiYesnoTypes" placeholder="请选择申请状态">
                        <el-option label="同意" value="2"></el-option>
                        <el-option label="拒绝" value="3"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="审核回复">
                    <el-input type="textarea" v-model="form.xiangmuTuzhiYesnoText" placeholder="审核回复"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="xiangmuTuzhiYesnoTypesVisible = false">取 消</el-button>
                <el-button type="primary" @click="xiangmuTuzhiYesnoTypesShenhe">审 核</el-button>
            </div>
        </el-dialog>
        <!-- 条件查询 -->
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '图纸名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.xiangmuTuzhiName" placeholder="图纸名称" clearable></el-input>
                     </el-form-item>
                 
                     <el-form-item :label="contents.inputTitle == 1 ? '图纸类型' : ''">
                        <el-select v-model="searchForm.xiangmuTuzhiTypes" placeholder="请选择图纸类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in xiangmuTuzhiTypesSelectSearch"
                               v-bind:key="index"
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '申请状态' : ''">
                        <el-select v-model="searchForm.xiangmuTuzhiYesnoTypes" placeholder="请选择申请状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in xiangmuTuzhiYesnoTypesSelectSearch"
                               v-bind:key="index"
                               :label="item.indexName"
                               :value="item.codeIndex">
                            <!--lable是要展示的名称-->
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('xiangmuTuzhi','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/chuanbo/upload/xiangmuTuzhiMuBan.xls"
                        >批量导入项目图纸数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('xiangmuTuzhi','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="xiangmuTuzhiUploadSuccess"
                                :on-error="xiangmuTuzhiUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('xiangmuTuzhi','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入项目图纸数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('xiangmuTuzhi','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "xiangmuTuzhi.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
...
<!--                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="yuangongPhoto"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xiangmuTuzhiUuidNumber"
                                   header-align="center"
                                   label="图纸编号">
                        <template slot-scope="scope">
                            {{scope.row.xiangmuTuzhiUuidNumber}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xiangmuTuzhiName"
                                   header-align="center"
                                   label="图纸名称">
                        <template slot-scope="scope">
                            {{scope.row.xiangmuTuzhiName}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="xiangmuTuzhiFile"
                                     header-align="center"
                                     width="200"
                                     label="图纸文件">
                        <template slot-scope="scope">
                            <div v-if="scope.row.xiangmuTuzhiFile">
                                <a type="text" style="text-decoration:none" class="el-button" :href="$base.url+scope.row.xiangmuTuzhiFile">下载</a>
                            </div>
                            <div v-else>无文件</div>
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xiangmuTuzhiTypes"
                                      header-align="center"
                                      label="图纸类型">
                        <template slot-scope="scope">
                            {{scope.row.xiangmuTuzhiValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xiangmuTuzhiYesnoTypes"
                                      header-align="center"
                                      label="申请状态">
                        <template slot-scope="scope">
                            {{scope.row.xiangmuTuzhiYesnoValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xiangmuTuzhiYesnoText"
                                      header-align="center"
                                      label="审核回复">
                        <template slot-scope="scope">
                            <span v-if="scope.row.xiangmuTuzhiYesnoText != null &&scope.row.xiangmuTuzhiYesnoText.length>10">
                                {{scope.row.xiangmuTuzhiYesnoText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.xiangmuTuzhiYesnoText}}
                            </span>
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xiangmuTuzhiShenheTime"
                                   header-align="center"
                                   label="审核时间">
                        <template slot-scope="scope">
                            {{scope.row.xiangmuTuzhiShenheTime}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('xiangmuTuzhi','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('xiangmuTuzhi','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('xiangmuTuzhi','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('xiangmuTuzhi','审核') && scope.row.xiangmuTuzhiYesnoTypes == 1 " type="primary" icon="el-icon-thumb" size="mini" @click="openYesnoTypes(scope.row.id)">审核</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              xiangmuTuzhiTypesSelectSearch : [],
              xiangmuTuzhiYesnoTypesSelectSearch : [],
            form:{
                id : null,
                xiangmuId : null,
                yuangongId : null,
                xiangmuTuzhiUuidNumber : null,
                xiangmuTuzhiName : null,
                xiangmuTuzhiFile : null,
                xiangmuTuzhiTypes : null,
                xiangmuTuzhiContent : null,
                insertTime : null,
                xiangmuTuzhiYesnoTypes : null,
                xiangmuTuzhiYesnoText : null,
                xiangmuTuzhiShenheTime : null,
                createTime : null,
            },
            dataList: [],
            pageIndex: 1,
            ...
            xiangmuTuzhiYesnoTypesVisible : false,
            ...
            json_fields: {
                //级联表字段
                     ...
                     '账户状态': 'jinyongTypes',
                //本表字段
                     '图纸编号': "xiangmuTuzhiUuidNumber",
                     '图纸名称': "xiangmuTuzhiName",
                     '图纸文件': "xiangmuTuzhiFile",
                     '图纸类型': "xiangmuTuzhiValue",
                     '上传时间': "insertTime",
                     '申请状态': "xiangmuTuzhiYesnoValue",
                     '审核回复': "xiangmuTuzhiYesnoText",
                     '审核时间': "xiangmuTuzhiShenheTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'xiangmu_tuzhi',//当前表表名,
                        sumColum : 'xiangmu_tuzhi_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'xiangmu_tuzhi_name',//分组字符串字段
                        // types : 'xiangmu_tuzhi_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "xiangmu_tuzhi_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.xiangmuTuzhiName!= '' && this.searchForm.xiangmuTuzhiName!= undefined) {
                    params['xiangmuTuzhiName'] = '%' + this.searchForm.xiangmuTuzhiName + '%'
                }
                 
                if (this.searchForm.xiangmuTuzhiTypes!= '' && this.searchForm.xiangmuTuzhiTypes!= undefined) {
                    params['xiangmuTuzhiTypes'] = this.searchForm.xiangmuTuzhiTypes
                }
                         
                if (this.searchForm.xiangmuTuzhiYesnoTypes!= '' && this.searchForm.xiangmuTuzhiYesnoTypes!= undefined) {
                    params['xiangmuTuzhiYesnoTypes'] = this.searchForm.xiangmuTuzhiYesnoTypes
                }
                        
                params['xiangmuTuzhiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "xiangmuTuzhi/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=xiangmu_tuzhi_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.xiangmuTuzhiTypesSelectSearch = data.data.list;
                    }
                });
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=xiangmu_tuzhi_yesno_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.xiangmuTuzhiYesnoTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            xiangmuTuzhiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "xiangmuTuzhi/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            openYesnoTypes(id) {
              ...
              _this.form.id = null;
                _this.form.id = id;
                            _this.form.xiangmuTuzhiYesnoText = null;
                _this.form.xiangmuTuzhiYesnoTypes = "请选择申请状态";
                _this.xiangmuTuzhiYesnoTypesVisible = true;
            },

            xiangmuTuzhiYesnoTypesShenhe() {
                let _this = this;
                if(_this.form.xiangmuTuzhiYesnoTypes == "请选择申请状态"){
                    _this.$message.error("请选择申请状态");return false;
                }
                if(_this.form.xiangmuTuzhiYesnoText == null || _this.form.xiangmuTuzhiYesnoText == ""){
                    _this.$message.error("请输入审核回复");return false;
                }
                this.$http({
                    url:`xiangmuTuzhi/shenhe`,
                    method: "post",
                    data: _this.form
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.$message({
                            ...
                            duration: 1500,
                            onClose: () => {
                                _this.xiangmuTuzhiYesnoTypesVisible = false;
                                _this.search();
                            }
                        });
                    } else {
