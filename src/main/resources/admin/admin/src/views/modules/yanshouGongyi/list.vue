            ...
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '工艺名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.yanshouGongyiName" placeholder="工艺名称" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('yanshouGongyi','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/chuanbo/upload/yanshouGongyiMuBan.xls"
                        >批量导入工艺验收数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('yanshouGongyi','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="yanshouGongyiUploadSuccess"
                                :on-error="yanshouGongyiUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('yanshouGongyi','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入工艺验收数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('yanshouGongyi','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "yanshouGongyi.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
...
<!--                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="yanshouGongyiName"
                                   header-align="center"
                                   label="工艺名称">
                        <template slot-scope="scope">
                            {{scope.row.yanshouGongyiName}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="yanshouGongyiBuwei"
                                   header-align="center"
                                   label="验收部位">
                        <template slot-scope="scope">
                            {{scope.row.yanshouGongyiBuwei}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="yanshouGongyiBiaozhun"
                                   header-align="center"
                                   label="使用标准">
                        <template slot-scope="scope">
                            {{scope.row.yanshouGongyiBiaozhun}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="yanshouGongyiShijishiyong"
                                   header-align="center"
                                   label="实际使用">
                        <template slot-scope="scope">
                            {{scope.row.yanshouGongyiShijishiyong}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('yanshouGongyi','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('yanshouGongyi','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('yanshouGongyi','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                ...
                xiangmuId : null,
                yuangongId : null,
                yanshouGongyiName : null,
                yanshouGongyiBuwei : null,
                yanshouGongyiBiaozhun : null,
                yanshouGongyiShijishiyong : null,
                jianzaojinduFile : null,
                hegeTypes : null,
                yanshouTime : null,
                yanshouGongyiContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '账户状态': 'jinyongTypes',
                //本表字段
                     '工艺名称': "yanshouGongyiName",
                     '验收部位': "yanshouGongyiBuwei",
                     '使用标准': "yanshouGongyiBiaozhun",
                     '实际使用': "yanshouGongyiShijishiyong",
                     '附件': "jianzaojinduFile",
                     '是否合格': "hegeValue",
                     '验收时间': "yanshouTime",
                     '录入时间': "insertTime",
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'yanshou_gongyi',//当前表表名,
                        sumColum : 'yanshou_gongyi_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'yanshou_gongyi_name',//分组字符串字段
                        // types : 'yanshou_gongyi_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "yanshou_gongyi_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.yanshouGongyiName!= '' && this.searchForm.yanshouGongyiName!= undefined) {
                    params['yanshouGongyiName'] = '%' + this.searchForm.yanshouGongyiName + '%'
                }
                ...
                params['yanshouGongyiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "yanshouGongyi/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            yanshouGongyiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "yanshouGongyi/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "yanshouGongyi/update",
                        method: "post",
                        data: {
                            id:id,
//                            yanshouGongyiTypes:yanshouGongyiTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
