                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           ...
                           href="http://localhost:8080/chuanbo/upload/xiangmuMuBan.xls"
                        >批量导入项目数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('xiangmu','导入导出')"
...
<!--                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="yonghuPhoto"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xiangmuKucunNumber"
                                      header-align="center"
                                      label="监造数量">
                        <template slot-scope="scope">
                            {{scope.row.xiangmuKucunNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <!--<el-button v-if="isAuth('xiangmu','修改')" type="primary" icon="el-icon-add-location" size="mini" @click="plusXiangmuKucunNumber(scope.row.id,scope.row.xiangmuKucunNumber)">增加监造数量</el-button>-->
                            <!--<el-button v-if="isAuth('xiangmu','修改')" type="primary" icon="el-icon-delete-location" size="mini" @click="reduceXiangmuKucunNumber(scope.row.id,scope.row.xiangmuKucunNumber)">减少监造数量</el-button>-->
...
<!-- 库存 -->
		<el-dialog
                :title="xiangmuKucunNumberTitle"
                :visible.sync="xiangmuKucunNumberVisible"
                width="30%">
            <span>操作数量</span><el-input-number v-model="kucunNumber" :min="1" :max="100" label="描述文字"></el-input-number>
            <span slot="footer" class="dialog-footer">
			<el-button @click="xiangmuKucunNumberVisible = false">取 消</el-button>
			<el-button type="primary" @click="addXiangmuKucunNumber()">确 定</el-button>
		  </span>
        </el-dialog>
...
<script>
    ...
    //当前表下拉框搜索条件
        ...
        //库存
            ...
            //原有数量
            xiangmuKucunNumber: 0,
            //标题
            xiangmuKucunNumberTitle: null,
            //模态框状态
            xiangmuKucunNumberVisible: false,
            };
        },
        created() {
        ...
        methods: {
            ...
            //增加监造数量
			plusXiangmuKucunNumber(thisId,xiangmuKucunNumber){
			//清空数据
				//监造数量数据设置为0
				this.kucunNumber = 0
				//id清空
			...
			//数据不为空放值
				...
				//将模态框打开
				this.xiangmuKucunNumberVisible = true;
				//标题
				this.xiangmuKucunNumberTitle = '增加监造数量'
			},
			//减少监造数量
			reduceXiangmuKucunNumber(thisId,xiangmuKucunNumber){
			//清空数据
				//监造数量数据设置为0
				this.kucunNumber = 0
				//id清空
			...
			//数据不为空放值
				...
				this.xiangmuKucunNumberVisible = true;
				//标题
				this.xiangmuKucunNumberTitle = '减少监造数量'
			},
			//提交用户操作监造数量数量
			addXiangmuKucunNumber(){
				var type = null
				var maxNumber = 0
				if(this.kucunNumberTypes == "+"){
					type = '增加'
					maxNumber = this.xiangmuKucunNumber + this.kucunNumber
				}else{
					type = '减少'
					if(this.xiangmuKucunNumber < this.kucunNumber || this.kucunNumber <= 0){
						this.$message.error('监造数量不足不能减少');
						return false;
					}else{
						maxNumber = this.xiangmuKucunNumber - this.kucunNumber
					}
				...
				this.xiangmuKucunNumberVisible = false;
			},	
            //无用
            wuyong(id) {
                let _this = this;
