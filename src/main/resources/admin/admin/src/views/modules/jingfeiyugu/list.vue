            ...
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '材料名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.jingfeiyuguName" placeholder="材料名称" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('jingfeiyugu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/chuanbo/upload/jingfeiyuguMuBan.xls"
                        >批量导入经费预估数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('jingfeiyugu','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="jingfeiyuguUploadSuccess"
                                :on-error="jingfeiyuguUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('jingfeiyugu','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入经费预估数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('jingfeiyugu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "jingfeiyugu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
...
<!--                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jingfeiyuguUuidNumber"
                                   header-align="center"
                                   label="经费预估编号">
                        <template slot-scope="scope">
                            {{scope.row.jingfeiyuguUuidNumber}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jingfeiyuguName"
                                   header-align="center"
                                   label="材料名称">
                        <template slot-scope="scope">
                            {{scope.row.jingfeiyuguName}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jingfeiyuguDanwei"
                                   header-align="center"
                                   label="单价单位">
                        <template slot-scope="scope">
                            {{scope.row.jingfeiyuguDanwei}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jingfeiyuguDanjia"
                                   header-align="center"
                                   label="材料单价">
                        <template slot-scope="scope">
                            {{scope.row.jingfeiyuguDanjia}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="cailiaoTypes"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="jingfeiyuguNumber"
                                      header-align="center"
                                      label="所需数量">
                        <template slot-scope="scope">
                            {{scope.row.jingfeiyuguNumber}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jingfeiyuguZongjia"
                                   header-align="center"
                                   label="预计总价">
                        <template slot-scope="scope">
                            {{scope.row.jingfeiyuguZongjia}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jingfeiyuguGongsi"
                                   header-align="center"
                                   label="推荐公司">
                        <template slot-scope="scope">
                            {{scope.row.jingfeiyuguGongsi}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="jingfeiyuguFile"
                                     header-align="center"
                                     width="200"
                                     label="执行标准">
                        <template slot-scope="scope">
                            <div v-if="scope.row.jingfeiyuguFile">
                                <a type="text" style="text-decoration:none" class="el-button" :href="$base.url+scope.row.jingfeiyuguFile">下载</a>
                            </div>
                            <div v-else>无文件</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('jingfeiyugu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('jingfeiyugu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('jingfeiyugu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                ...
                xiangmuId : null,
                yuangongId : null,
                jingfeiyuguUuidNumber : null,
                jingfeiyuguName : null,
                jingfeiyuguDanwei : null,
                jingfeiyuguDanjia : null,
                cailiaoTypes : null,
                jingfeiyuguNumber : null,
                jingfeiyuguZongjia : null,
                jingfeiyuguGongsi : null,
                jingfeiyuguFile : null,
                jingfeiyuguContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '账户状态': 'jinyongTypes',
                //本表字段
                     '经费预估编号': "jingfeiyuguUuidNumber",
                     '材料名称': "jingfeiyuguName",
                     '单价单位': "jingfeiyuguDanwei",
                     '材料单价': "jingfeiyuguDanjia",
                     '材料类型': "cailiaoValue",
                     '所需数量': "jingfeiyuguNumber",
                     '预计总价': "jingfeiyuguZongjia",
                     '推荐公司': "jingfeiyuguGongsi",
                     '执行标准': "jingfeiyuguFile",
                     '上传时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'jingfeiyugu',//当前表表名,
                        sumColum : 'jingfeiyugu_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'jingfeiyugu_name',//分组字符串字段
                        // types : 'jingfeiyugu_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.jingfeiyuguName!= '' && this.searchForm.jingfeiyuguName!= undefined) {
                    params['jingfeiyuguName'] = '%' + this.searchForm.jingfeiyuguName + '%'
                }
                ...
                params['jingfeiyuguDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "jingfeiyugu/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            jingfeiyuguUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "jingfeiyugu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "jingfeiyugu/update",
                        method: "post",
                        data: {
                            id:id,
//                            jingfeiyuguTypes:jingfeiyuguTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
